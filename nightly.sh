...
then
    ...
    cd packages
    mv `ls | grep "falcon-desktop.*linux-x64.zip"` falcon-desktop-linux-x64-PR$TRAVIS_PULL_REQUEST-$TRUE_COMMIT.zip
    mv `ls | grep "falcon-desktop.*linux-amd64.deb"` falcon-desktop-linux-amd64-PR$TRAVIS_PULL_REQUEST-$TRUE_COMMIT.deb
    cd ..
...
then
    ...
    cd packages
    mv `ls | grep "falcon-desktop.*mac.zip"` falcon-desktop-mac-PR$TRAVIS_PULL_REQUEST-$TRUE_COMMIT.zip
    mv `ls | grep "falcon-desktop.*mac.dmg"` falcon-desktop-mac-PR$TRAVIS_PULL_REQUEST-$TRUE_COMMIT.dmg
    cd ..
...
#    echo disable > "${WINEPREFIX:-${HOME}/.wine}/.update-timestamp"
    ...
    cd packages
    zip -r falcon-desktop-win-x64-PR$TRAVIS_PULL_REQUEST-$TRUE_COMMIT.zip win-unpacked
    cd ..

    DEBUG=electron-builder yarn run travis:win32

    cd packages
    zip -r falcon-desktop-win-ia32-PR$TRAVIS_PULL_REQUEST-$TRUE_COMMIT.zip win-ia32-unpacked
    cd ..
...
then
    ...
    for fn in `ls | grep "falcon-desktop"`; do
        echo "Uploading $fn"
