...
const conFilePath = path.join( cookie.getGhostPath(_options), 'falcon.conf');
...
const readConfigFile = function () {
  log.debug('Attempting to read falcond config from: ', conFilePath);
  if (fs.existsSync(conFilePath)) {
    try {
      ...
      log.error(`falcond config file parsing failed from ${conFilePath}`);
      ...
      if (err) {
        ...
        fs.rename(tmpConfPath, conFilePath, (error) => {
          if (error) {
            ...
            log.info('Successfully set falcond configuration at', conFilePath);
          }
        });
      }
    });
